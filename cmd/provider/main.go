...
func main() { //nolint:funlen
	var (
		app = kingpin.New(
			...
			"Terraform based Crossplane provider for ProviderMongoDBAtlas",
		).DefaultEnvars()
		debug = app.Flag("debug", "Run with debug logging.").Short('d').Bool()

		enableExternalSecretStores = app.Flag(
			"enable-external-secret-stores", "Enable support for ExternalSecretStores.",
		).Default("false").Envar("ENABLE_EXTERNAL_SECRET_STORES").Bool()

		enableManagementPolicies = app.Flag(
			"enable-management-policies", "Enable support for Management Policies.",
		).Default("true").Envar("ENABLE_MANAGEMENT_POLICIES").Bool()

		essTLSCertsPath = app.Flag(
			"ess-tls-cert-dir", "Path of ESS TLS certificates.",
		).Envar("ESS_TLS_CERTS_DIR").String()

		leaderElection = app.Flag(
			"leader-election", "Use leader election for the controller manager.",
		...
		namespace = app.Flag(
			"namespace", "Namespace used to set as default scope in default secret store config.",
		...
		pollStateMetricInterval = app.Flag(
			"poll-state-metric", "State metric recording interval",
		).Default("5s").Duration()

		providerSource = app.Flag(
			"terraform-provider-source", "Terraform provider source.",
		).Required().Envar("TERRAFORM_PROVIDER_SOURCE").String()

		providerVersion = app.Flag(
			"terraform-provider-version", "Terraform provider version.",
		).Required().Envar("TERRAFORM_PROVIDER_VERSION").String()

		syncPeriod = app.Flag(
			"sync", "Controller manager sync period such as 300ms, 1.5h, or 2h45m",
		).Short('s').Default("1h").Duration()

		terraformVersion = app.Flag(
			"terraform-version", "Terraform version.",
		).Required().Envar("TERRAFORM_VERSION").String()
	)
